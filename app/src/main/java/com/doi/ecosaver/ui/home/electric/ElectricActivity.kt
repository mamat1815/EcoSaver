...
import com.doi.ecosaver.databinding.ActivityElectricBinding
...
class ElectricActivity : AppCompatActivity() {
    ...
    private lateinit var viewModel: ElectricActivityViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this, viewModelFactory)[ElectricActivityViewModel::class.java]
        ...
        binding.apply {
            viewModel.apply {
                ...
                getDetail(idUser, currentMonthYear).observe(this@ElectricActivity) { it ->
                    current = it?.kwh?.toFloat() ?: 0f
                    tvDetail.text = "${current?.toInt() ?: 0}kWh"
                }

                getDetail(idUser, lastMonthYear).observe(this@ElectricActivity) { it ->
                    last = it?.kwh?.toFloat() ?: 0f
                    tvDetailVs.text = "vs ${last?.toInt() ?: 0}kWh"
                }
            }
        }

        viewModel.listrikDataList.observe(this) { list ->
            showLineChart(list)
            val prompt =
    ...
    private fun showLineChart(data: List<ListrikDetail>) {
        val entries = data.mapIndexed { index, item ->
            Entry(index.toFloat(), item.kWh)
        }

        val dataSet = LineDataSet(entries, "kWh Listrik per Bulan").apply {
            color = Color.BLUE
        ...
        binding.chart.apply {
            ...
            description.text = "Grafik Pemakaian Listrik"
