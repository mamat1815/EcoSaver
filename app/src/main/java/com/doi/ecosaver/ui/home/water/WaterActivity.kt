...
import com.doi.ecosaver.databinding.ActivityWaterBinding
...
class WaterActivity : AppCompatActivity() {
    ...
    private lateinit var viewModel: WaterActivityViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this, viewModelFactory)[WaterActivityViewModel::class.java]
        ...
        binding.apply {
            viewModel.apply {
                ...
                getAirDetail(idUser, currentMonthYear).observe(this@WaterActivity) { air ->
                    currentAir = air?.liter?.toFloat() ?: 0f
                    tvDetail.text = "${currentAir?.toInt() ?: 0}L"
                }

                getAirDetail(idUser, lastMonthYear).observe(this@WaterActivity) { air ->
                    lastAir = air?.liter?.toFloat() ?: 0f
                    tvDetailVs.text = "vs ${lastAir?.toInt() ?: 0}L"
                }
            }
        }

        viewModel.airDataList.observe(this) { list ->
        ...
        viewModel.loadAirData()



    }

    private fun showLineChart(data: List<AirDetail>) {
        val entries = data.mapIndexed { index, item ->
            Entry(index.toFloat(), item.liter)
        }

        val dataSet = LineDataSet(entries, "Liter Air per Bulan").apply {
            color = Color.BLUE
        ...
        binding.chartFuel.apply {
            ...
            description.text = "Grafik Pemakaian Air"
